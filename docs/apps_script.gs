 ...
 * column order). New proposals default to Estado="Aprobado" (auto-approved
 * — the only automatic safety net is the lat/lon range check in
 * scripts/sync_sites.py); flip a row to "Rechazado"/"Pendiente" by hand to
 * pull a site back out.
 */
...
function doPost(e) {
  try {
    ...
    } else if (payload._kind === "site_sync_backfill") {
      handleSiteSyncBackfill(payload);
    } else {
      handleAlert(payload);
    }
...
function handleSiteProposal(payload) {
  ...
  sheet.appendRow([
    ...
    payload.comentarios || "",
    "Aprobado", // auto-aprobado: la única red de seguridad automática es la validación de rango lat/lon en sync_sites.py
    "",
    "",
  ]);
}
...
/**
 * Called by scripts/sync_sites.py after every sync so the autogenerated
 * Site ID / Timezone (and, if set, Modelo) show up in the sheet itself —
 * purely cosmetic, sync_sites.py already computed and used these values
 * before calling this; it does NOT re-read them back from the sheet.
 * Only touches rows that are blank in that column, never overwrites a
 * value the admin typed by hand.
 * payload.updates: [{row, site_id, timezone, model}, ...] (row = sheet row number, 1-based, from the same CSV export sync_sites.py read).
 */
function handleSiteSyncBackfill(payload) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(PROPOSALS_SHEET_NAME);
  if (!sheet) return;

  const idColIdx = PROPOSALS_HEADERS.indexOf("Site ID") + 1;
  const tzColIdx = PROPOSALS_HEADERS.indexOf("Timezone") + 1;
  const modeloColIdx = PROPOSALS_HEADERS.indexOf("Modelo") + 1;

  (payload.updates || []).forEach((u) => {
    if (!u.row || u.row < 2 || u.row > sheet.getLastRow()) return;
    const idCell = sheet.getRange(u.row, idColIdx);
    const tzCell = sheet.getRange(u.row, tzColIdx);
    const modeloCell = sheet.getRange(u.row, modeloColIdx);
    if (!idCell.getValue() && u.site_id) idCell.setValue(u.site_id);
    if (!tzCell.getValue() && u.timezone) tzCell.setValue(u.timezone);
    if (!modeloCell.getValue() && u.model) modeloCell.setValue(u.model);
  });
}
